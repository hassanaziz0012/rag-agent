...
def insert_all_records():
    ...
    records = [
        {"id": str(i), "text": p, "book_title": "Purpose and Profit"}
...
results = index.search(
    ...
    rerank={"model": "bge-reranker-v2-m3", "top_n": 10, "rank_fields": ["text"]},
)

for r in results["result"]["hits"]:
    print(
        f"id: {r['_id']:<5} | score: {round(r['_score'], 2):<5} | book_title: {r['fields']['book_title']:<10} | text: {r['fields']['text']:<50}\n"
    )
